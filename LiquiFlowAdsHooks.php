	...
	public static function onParserBeforeStrip( &$parser, &$text, &$mStripState ) {
				...
				foreach($findings[1] as $findingid => $finding) {
					if(!$has_added_adbox) {
						if (trim($finding) == trim($key_heading, "* \t\n\r\0\x0B")) {
							$text = preg_replace('/' . preg_quote($findings[0][$findingid]) . '/', $findings[0][$findingid] . "\n" . $adbox_tag . "\n", $text);
		...
		if(!$has_added_adbox) {
			if (count($findings[0]) <= 2) {
				...
				$text = preg_replace('/' . preg_quote($findings[0][ceil((count($findings[0]) - 1) / 2)]) . '/', $findings[0][ceil((count($findings[0]) - 1) / 2)] . "\n" . $adbox_tag . "\n", $text);
				$has_added_adbox = true;
			}
		}
		return true;
